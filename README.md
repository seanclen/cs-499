...
2. Algorithms and Data Structures
    * Temporal Optimization: Originally, the Appointment Service was to be refactored to replace its linear search array with a Self-Balancing Binary Search Tree (TreeMap). This specialized data structure reduces the time complexity of critical temporal range queries (e.g., checking for scheduling conflicts) from O(n) to O(log n). While the TreeMap provides O(log n) complexity, a HashMap instead offers O(1) (constant time) average complexity for findById, save, and deleteById. This is generally faster than O(log n), especially for large repositories, because it uses hashing to directly calculate the location of the element.
